...
----------------------------------------------------------------------------------------------
	Summary:
		Runs as an inbound action on a queue, only runs for items routed from specific queue(s)
		Will remap keys between index values and custom properties within a single document.

		YAML filename (except for extension) should equal the name of the queue
...
var EMAIL_ERRORS_NOTIFY = "user@umass.edu";
...
function main() {
	try {
		...
		loadYAMLConfig(imagenowDir6+"\\script\\Restore_Keys\\config\\");
		...
		for (i = history.length - 1; i >= 0; i--) {
			if (history[i].stateDetail === 2) {
				for (i2 = CUR_CFG.previousQueues.length - 1; i2 >= 0; i2--) {
					if (CUR_CFG.previousQueues[i2] === history[i].queueName) {
						runRemap = true;
						debug.log('DEBUG', 'was routed from a configured queue, remapping will commence now.\n');
						break;
					}
				}
			}
		...
		if (!runRemap) {
			debug.log('DEBUG', 'Item routed from a queue not included in the configuration. Will not attempt remapping\n');
			return false;
		}
